...
| `confirm_r3_green.py` | Drive VP_r3's surviving HA entity R/G/B directly and measure camera margins | Confirmed VP_r3's s4 green MISS was optical (green +3.5 just below the +4 floor as afternoon ambient rose), not an agent fault (red +78 / blue +8.8) -> classified FT8 not FT5 |
| `_classify_vp_r3.py` | Operator classification of T1_CLD_VP_r3 as FT8 (void) | Green works but is LED3's dimmest channel; bright afternoon ambient sank it below the detection floor -- a measurement limit, re-run under better light |
| `test_clock_detection.py` | T2 pre-flight: run yolov8n on a board-camera frame to check if the target is detected as COCO "clock" | The LED 7-segment desk clock is NOT detected as "clock" (only chair/couch/mouse at low conf) -- COCO clock is trained on analog clocks; T2 target must change to a reliably-detected class (cell phone / bottle / person) |
